...
const char *gfx_frag =
         ...
         "{"
           ...
           "for(int _note=_noteL;_note<=_noteU;_note++)"
             "{"
               ...
               "if(Bdrum!=0)"
                 "d=max(d,1.-smoothstep(Bon,Bon+.1,B)),iNBeats+=1.;"
             "}"
           "return d;"
         "}"
     "}"
 ...
 "{"
   "vec2 sdf=vec2(x.z,7.);"
   "float endeavor=dstring(x.xy+2.*(-6.+1.2*iTime-16.8)*c.xy,0.,.8);"
 ...
 "{"
   ...
   "float da=.5*snoise_2d(2.*vec2(x.x,abs(x.y+.15))-.4*iTime),dx=da+mfsnoise_2d(vec2(x.x-.2*iTime,abs(x.y+.15)),1.,1000.,.55);"
   ...
   "dx=da+mfsnoise_2d(vec2(x.x-.1*iTime-15.,abs(x.y+.15)),1.,1000.,.55);"
         ...
         "else"
           " if(iTime<28.)"
             "{"
               "vec3 c1=c.yyy;"
               "camerasetup(camera0,ro,r,u,t,uv,dir);"
               "raymarch(textpre2,x,ro,d,dir,s,100,2e-05,hit);"
               "if(hit)"
                 "hit=false;"
               "else"
                 " d=-ro.z/dir.z;"
               "raymarch(texteffect2,x,ro,d,dir,s,200,2e-05,hit);"
               "if(hit)"
                 "{"
                   "vec3 n;"
                   "calcnormal(scene,n,.0002,x);"
