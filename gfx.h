...
const char *gfx_frag =
 ...
 "{"
   ...
   "if(ordinal>=nstrings)"
     "return 1.;"
   "float stroff=floor(rfloat(stroff0+1.+2.*ordinal)),len=floor(rfloat(stroff0+2.+2.*ordinal)),d=1.;"
   "for(float i=0.;i<len;i+=1.)"
     "d=min(d,dglyph(x-2.1*i*size*c.xy,floor(rfloat(stroff+i)),.8*size));"
   "return d;"
 "}"
         ...
         "if(iTime<1000.)"
           "{"
             ...
             "if(d==1.)"
               "col+=c.yxy;"
             "else"
               " d=stroke(d,.01),col+=mix(c.yyy,c.xyy,smoothstep(-1.5/iResolution.y,1.5/iResolution.y,d));"
           "}"
         "else"
           " if(iTime<28.)"
             "col+=background2(uv);"
