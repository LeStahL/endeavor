...
const char *gfx_frag =
 ...
 "{"
   "float max_mod_off=4.;"
   "int drum_index=24;"
   "float drum_synths=10.,r=0.,d=0.,BT=mod(BPS*t*44100.,max_mod_off);"
   "if(BT>max_mod_off)"
     "return r;"
         ...
         "{"
           ...
           "for(int _note=_noteL;_note<=_noteU;_note++)"
             "{"
               ...
               "float BT1=mod(BPS*t*44100.,.2*BPS*44100.)-BPS*.1*44100.,env=mix(Bon,Boff,.5);"
               "env=smoothstep(env-4410.,env,BT1);"
               "env=min(env,1.-smoothstep(env,env+4410.,BT1));"
               "d=max(d,env);"
             "}"
           "return.5*d;"
         "}"
